...
import { CheckIcon, Star, Settings, ExternalLink, ArrowLeft } from "lucide-react";
    ...
    try {
      ...
      const response = await fetch('/api/subscription-checkout', {
        ...
        body: JSON.stringify({
          ...
          cancelUrl: `${window.location.origin}/m/dashboard?canceled=true`,
        }),
      });
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="px-4 py-6 space-y-6">
        ...
        {/* 権限エラー通知 */}
        {permissionError && (
          <Card className="p-4 border border-yellow-400 bg-yellow-50">
            <h3 className="font-bold text-yellow-800 text-sm">Supabase権限エラーが発生しています</h3>
          ...
          {subscriptionPlans.map((plan: StripeProduct, index) => {
            ...
            return (
              ...
              >
                {/* おすすめバッジ */}
                {plan.recommended && (
                  <div className="bg-green-500 text-white px-3 py-1 text-xs font-bold flex items-center justify-center gap-1">
                ...
                {/* 現在のプランバッジ */}
                {isCurrentPlan && (
                  <div className="bg-blue-500 text-white px-3 py-1 text-xs font-bold text-center">
                    現在のプラン
                  </div>
                )}
                ...
                <div className={`p-4 ${
                    ...
                    <ul className="space-y-3">
                      {plan.features.map((feature, fIndex) => (
                        <li key={fIndex} className="flex items-start">
                          <CheckIcon className={`w-4 h-4 mr-3 flex-shrink-0 mt-0.5 ${
                            plan.recommended 
                              ? 'text-green-200' 
                              : isCurrentPlan 
                                ? 'text-blue-200'
                  ...
                  {isCurrentPlan ? (
                    ...
                    plan.priceId === 'free' ? (
                      <Button
                        className="w-full py-3 font-semibold bg-gray-100 text-gray-600 cursor-default"
                        disabled
                      >
                        現在のプラン
                      </Button>
                      ...
                      <Button
                        onClick={openBillingPortal}
                        className="w-full py-3 font-semibold bg-white text-blue-600 hover:bg-gray-100 shadow-lg flex items-center justify-center gap-2"
                      >
                        <Settings className="w-4 h-4" />
                        <span>プラン管理</span>
                      </Button>
                    ...
                    plan.priceId === 'free' ? (
                      <Button
                        className="w-full py-3 font-semibold bg-gray-100 text-gray-600 cursor-not-allowed"
                        disabled
                      >
                        ダウングレード不可
                      </Button>
