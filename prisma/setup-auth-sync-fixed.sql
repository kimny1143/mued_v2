-- ===================================
-- 認証ユーザー自動同期システム 単独セットアップ（修正版）
-- ===================================
-- 
-- このスクリプトは、既存のデータベースに認証同期システムのみを
-- 追加するためのものです。
-- 
-- 修正内容: auth.users.id (uuid) と public.users.id (text) の型キャスト対応
-- 
-- 実行前提条件:
...
BEGIN
    ...
    -- public.usersテーブルに挿入または更新（型キャスト対応）
...
BEGIN
    ...
    -- public.usersテーブルを更新（既存レコードのみ、型キャスト対応）
...
BEGIN
    ...
    RAISE NOTICE '  - UUID→TEXT型キャスト対応';
    RAISE NOTICE '';
    RAISE NOTICE '🔧 動作確認:';
    RAISE NOTICE '  SELECT * FROM public.test_user_sync();';
    RAISE NOTICE '';
    RAISE NOTICE '📝 詳細確認:';
    RAISE NOTICE '  SELECT au.email, pu.name, pu."roleId" FROM auth.users au JOIN public.users pu ON au.id::text = pu.id;';
    RAISE NOTICE '';
