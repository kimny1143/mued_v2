...
| 機能 | Phase | 理由・備考 |
|-----|-------|-----------|
| **hum（鼻歌）検出** | Phase 2 PoC | 理念「欲の記録」に対応。音声分類モデル要検証 |
| **音声素材保存** | Phase 2 | hum 検出後に実装 |
| Android対応 | Phase 2 | iOS 優先 |
| プッシュ通知 | Phase 2 | MVP ではシンプルに |
| オフライン同期 | Phase 2 | 保存の後回し機能 |
| HLA高度分析 | Phase 3 | 判断履歴の可視化ダッシュボード |

> **重要**: hum 検出は MUED 理念「欲の記録」（感覚的・本能的衝動の記録）に直結する機能。
> Phase 2 開始時に技術 PoC を実施し、音声分類モデル（speech/hum/music/noise）の実用性を検証する。
...
MUED理念に基づき、初回起動時に段階的に思想を伝える。

#### レイヤー0: タグライン（スプラッシュ画面）
...
#### レイヤー1: 要約カード（初回起動時スライダー）

| カード | 見出し | 本文 |
|-------|-------|------|
| 1 | **AIは思考の鏡である** | AIは人間の脳の動きを外部に可視化し、加速させたもの。思考の代替ではなく拡張。 |
| 2 | **創作とは「選び続けること」** | AI時代において、創作とは「作ること」ではなく「選び続けること」。 |
| 3 | **反応を記録しよう** | 違和感も、納得感も。どちらもあなたの判断の痕跡です。 |

#### レイヤー2: 完全版（設定画面からリンク）

- `/about` または `/philosophy` ページへリンク
- `docs/concept/MUED理念本文_決定稿.md` の内容を表示

### 4.5 レビュー画面の拡張

理念「捨てた理由を記録する」に対応するため、レビュー画面を拡張。

#### 追加要素

| 要素 | 必須/任意 | 説明 |
|-----|----------|------|
| **セッションメモ** | 任意 | 「このセッションで気づいたこと」自由記述 |
| **保存理由** | 任意 | 「なぜこの思考を残す？」短文入力 |
| **マイクロコピー** | 表示 | 保存ボタン横に「反応を記録しよう」 |
