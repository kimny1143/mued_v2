  ...
  if (customerId) {
    try {
      ...
      if (subscriptions.data.length > 0) {
        ...
        // 同じ通貨（JPY）内でのプラン変更
        console.log('🔄 JPY内でのプラン変更処理 - 既存サブスクリプションを更新');
        
        try {
          // 既存のサブスクリプションアイテムを新しい価格IDに更新
          ...
          await stripe.subscriptions.update(existingSub.id, {
            ...
            proration_behavior: 'create_prorations', // 日割り計算を有効にする
            metadata: {
              ...existingSub.metadata,
          ...
          console.log('✅ サブスクリプションプラン変更完了:', {
            subscriptionId: existingSub.id,
            oldPriceId: currentPriceId,
            newPriceId: priceId
          });

          // プラン変更完了後、成功ページにリダイレクト
          return {
            id: `updated_${existingSub.id}`,
            url: successUrl,
            metadata: {
              plan_updated: 'true',
              old_price_id: currentPriceId,
          ...
          // 更新に失敗した場合は、新しいサブスクリプションを作成
          console.log('🔄 更新失敗のため新規サブスクリプション作成にフォールバック');
          ...
          await stripe.subscriptions.update(existingSub.id, {
            ...
            metadata: {
              ...existingSub.metadata,
              cancellation_reason: 'plan_change_fallback',
          ...
          const session = await stripe.checkout.sessions.create({
            ...
            metadata: {
              ...metadata,
              plan_change_type: 'replacement',
              old_subscription_id: existingSub.id,
            },
            subscription_data: {
              metadata: {
                replaced_subscription: existingSub.id,
                plan_change_reason: 'update_failed',
              },
            },
          });
